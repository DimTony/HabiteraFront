...
export function EditProperty({
      ...
      try {
        ...
        if (response?.statusCode === 200 && response.data) {
          ...
          setForm({
            ...
            listingType: data.listingType ?? "ForSale",
            tenor: data.tenor ?? "Monthly",
            street: data.street ?? "",
            city: data.city ?? "",
            state: data.state ?? "",
            ...
            amenities:
              data.amenitiesData && Object.keys(data.amenitiesData).length > 0
                ? {
                    category: "Interior" as AmenityCategory, // API doesn't seem to return category
                    amenities: data.amenitiesData,
                  }
                : {
                    category: "Interior" as AmenityCategory,
                    amenities: {},
                  },
  ...
  const handleSubmit = async () => {
    if (!isFormValid() || isSubmitting || !listingId) return;

    setIsSubmitting(true);
    try {
      const response =
        userRole === "Agent"
          ? await updateProperty(listingId, {
              ...form,
              saveAsDraft: false,
            } as any)
          : null;
  ...
  const handleSaveForLater = async () => {
    ...
    try {
      const response =
        userRole === "Agent"
          ? await updateProperty(listingId, { ...form, saveAsDraft: true } as any)
          : null;
    ...
    <div className="bg-primary text-white h-full flex flex-col">
      ...
      <div className="px-6 py-6">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-base font-semibold text-white">Edit Property</h1>
